let dataJson;	//从服务器获取的数据
let id;			//性别id
let page=1;  //传入的当前页
let newTotalPage=[0,0,0,0];			//新品总页数
let trendTotalPage=[0,0,0,0];;		//时尚总页数
newTotalPage[0]=parseInt($(".hnh").val());
trendTotalPage[0]=parseInt($(".thred").val());
let newCurrentNo =[0,0,0,0];			//新品当前页
let trendCurrentNo =[0,0,0,0];		//时尚当前页
newCurrentNo[0]=1;			//初始化时男士为第一页
trendCurrentNo[0]=1;
let  topUlLiIndex;				//热销子版块下标
let newUlLiIndex=0;					//新品下子版块下标
let trendUlLiIndex=0;				//时尚下子版块下标
let ajaxReady = false;
//调用显示轮播图函数
banner();
...
function pdataLoad(pro,parent_id) {
	...
	for ( let i = 0; i < pro.length; i++) {
		let w_id = pro[i].watch_id;
		let brand_href = pro[i].watchBrand.brandId;
		let brandName = pro[i].watchBrand.brandName;
		let country_name = pro[i].watchBrand.country.cName;
		let str="<li onclick='li_click("+w_id+")' class=\"watchLi\" goods-code=\"42471\" data-is-boutique=\"0\">\n" +
			...
			"<a href=\"/brand/doShop.xf/"+brand_href+"\"  class=\"s_shop\">"+(country_name+brandName)+"</a>\n" +
			"</div>\n" +
			"</div>\n" +
			"</li>"
		if(parent_id==1){
...
$(function(){
	...
	$(".tGud li").click(function () {
		let parent_id = parseInt($(this).parent().prev("span").attr("about"));		//获取父级id
		if (parent_id==1){
...
//分页点击方法,上一页
	...
	$(".btnJtp").click(function () {
		...
		let pageNum =parseInt( $(this).prev().val());
		let  thisName= $(this).attr("name");
		let  tPage = parseInt($(this).prev().prev().prev().text());
		if (isNaN(pageNum)){
			toastr.error("请输入数字!");
