...
function PageTurning(data,cPage,doc){
	...
	function dataLoad(sours) {
		...
		for ( var i = 0; i < 8; i++) {
			var str = "<li onclick=\"li_click("
					+ sours[i].watch_Id
					+ ");\">"
					+ "<a title="+sours[i].brands.brandName+"&nbsp;&nbsp;"+sours[i].watch_Name+">"
					+ "<i class='c__pTag'><span style=''></span></i>"
					+ "<div class=\"tImg\"><img src=\""+sours[i].watch_Image+"\"></div>"
					+ "<div class=\"tNm\">"
					+ (sours[i].brands.brandName + sours[i].watch_Name) + "</div>"
	...
	function BrandInit(jDoc,b_id){
		...
		function(data){
			for(var i=0;i<15;i++){
				if(i<data.length){
					var ddb="<dd onclick=''><a name="+data[i].brands.brand_id+"  title="+data[i].brands.dect+">"+data[i].brands.brandName+"</a></dd>";
					jDoc.append(ddb);
				}
			}
