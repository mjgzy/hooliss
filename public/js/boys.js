		...
		for ( var i =ci ; i < ci+8; i++) {
			if(data[i]!=null){
				var str = "<li onclick=\"li_click("
						+ data[i].w_id
						+ ");\">"
						+ "<a title="+data[i].brands.brandName+"&nbsp;&nbsp;"+data[i].watch_Name+">"
						+ "<i class='c__pTag'><span style=''></span></i>"
						+ "<div class=\"tImg\"><img src=\""+data[i].watch_Image+"\"></div>"
						+ "<div class=\"tNm\">"
						+ (data[i].brands.brandName + data[i].watch_Name) + "</div>"
	...
	function datasoure(soure) {
		...
		for ( var i = 0; i < 8; i++) {
			var str = "<li onclick=\"li_click("
					+ soure[i].watch_Id
					+ ");\">"
					+ "<a title="+soure[i].brands.brandName+"&nbsp;&nbsp;"+soure[i].watch_Name+">"
					+ "<i class='c__pTag'><span style=''></span></i>"
					+ "<div class=\"tImg\"><img src=\""+soure[i].watch_Image+"\"></div>"
					+ "<div class=\"tNm\">"
					+ (soure[i].brands.brandName + soure[i].watch_Name) + "</div>"
	...
	function BrandInit(jDoc,b_id){
		...
		function(data){
			for(var i=0;i<15;i++){
				if(i<data.length){
					var ddb="<dd name="+data[i].brand_id+" onclick='ddClick($(this).attr('name'))'><a   title="+data[i].dect+">"+data[i].brandName+"</a></dd>";
					jDoc.append(ddb);
				}
			}
